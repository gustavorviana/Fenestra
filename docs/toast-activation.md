...
By default, Windows toast notifications are fire-and-forget. If the user clicks a toast after the app has been closed, nothing happens. `AddWindowsToastActivation()` solves this by:
...
> **Note:** `AddWindowsToastActivation()` must be called after `AddWindowsToastNotifications()`.
...
public partial class App : FenestraApp
{
    ...
    {
        builder.Services.AddWindowsToastNotifications();
        builder.Services.AddWindowsToastActivation(Guid.Parse("A1B2C3D4-E5F6-7890-ABCD-EF1234567890"));
        builder.RegisterWindows();
    }
...
Once `AddWindowsToastActivation()` writes the `ToastActivatorCLSID` to the Start Menu shortcut, Windows routes all toast activations through COM for that AUMID. If the CLSID changes or `AddWindowsToastActivation()` is removed without updating the shortcut, the old shortcut still references the stale CLSID and **all WinRT toast events (Activated, Dismissed) will stop firing** -- not just background activation, but in-app events too.
...
If you change the CLSID or remove `AddWindowsToastActivation()`, you must also update or recreate the shortcut so the old CLSID is cleared.
...
public partial class App : FenestraApp
{
    ...
    {
        builder.Services.AddWindowsToastNotifications();
        builder.Services.AddWindowsToastActivation(Guid.Parse("64BD1DB5-C8C7-41D1-958F-30B13D3F18ED"));
        builder.RegisterWindows();
    }
...
### WpfFenestraBuilder Extensions

| Method | Description |
|---|---|
| `AddWindowsToastActivation()` | Enables toast activation with a CLSID derived from the AppId |
| `AddWindowsToastActivation(Guid)` | Enables toast activation with an explicit CLSID |
